...
func Build(db *gorm.DB, redisClient *redis.Client, objectStore *minio.Client, cfg config.Config) (*gin.Engine, error) {
	...
	secured.GET("/specimens/:id/aliquot-tubes", aliquotHandler.List)
	secured.POST("/specimens/:id/aliquot-tubes", middleware.RequirePermission("specimen:transition"), aliquotHandler.Register)
