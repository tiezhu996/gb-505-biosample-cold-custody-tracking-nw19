...
1. 接收专员登记样本接收号、脱敏受试者编码、来源协议、接收体积和当前保管人。
2. 接收专员按冻存管登记分装：逐管填写管编号和每管体积；一次登记各管合计体积不得超过父样本当前剩余体积，超出则整批不写入，父样本体积、管数和状态保持原样。登记成功后父样本状态变为已分装，剩余体积扣减各管之和，管数累加。已冻存、放行或销毁的样本不再允许登记分装。
...
| `POST /api/specimens/:id/transition` | 销毁等处置状态变更（放行仅限协议复核） | `specimen:transition` |
| `GET /api/specimens/:id/aliquot-tubes` | 查询样本冻存管台账 | 已登录 |
| `POST /api/specimens/:id/aliquot-tubes` | 整批登记分装冻存管编号与每管体积 | `specimen:transition` |
...
冻存管整批登记示例（同一父样本下管编号唯一；合计体积超过剩余体积时整批不写入）：

```bash
curl -s -X POST http://localhost:19505/api/specimens/4/aliquot-tubes \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"tubes":[{"tubeCode":"BIO-20260822-004-T01","volumeMl":1.5},{"tubeCode":"BIO-20260822-004-T02","volumeMl":2.0}]}'
```

样本的 `volumeMl` 为当前剩余体积，`initialVolumeMl` 为接收时体积，`aliquotCount` 为已登记冻存管数；已冻存、放行或销毁的样本调用该接口返回 `409`。
