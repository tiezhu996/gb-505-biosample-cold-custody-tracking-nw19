...
import type { ProtocolReview, Specimen, SpecimenAliquot } from '../types/domain'
...
export function SpecimenDetailPage() {
  ...
  return (
    <div className="page-stack">
      ...
      <section className="detail-section"><Typography.Title level={4}>样本信息</Typography.Title><Descriptions bordered size="small" column={{ xs: 1, md: 3 }}><Descriptions.Item label="类型">{specimen.sampleType}</Descriptions.Item><Descriptions.Item label="受试者编码">{specimen.subjectCode}</Descriptions.Item><Descriptions.Item label="来源协议">{specimen.protocolCode}</Descriptions.Item><Descriptions.Item label="保管人">{specimen.currentCustodian}</Descriptions.Item><Descriptions.Item label="剩余体积/管数">{specimen.volumeMl} mL / {specimen.aliquotCount} 管</Descriptions.Item><Descriptions.Item label="接收时间">{formatDateTime(specimen.receivedAt)}</Descriptions.Item><Descriptions.Item label="冻存容器">{specimen.storageContainer?.name || '待分配'}</Descriptions.Item><Descriptions.Item label="格位">{specimen.position || '-'}</Descriptions.Item><Descriptions.Item label="温区">{specimen.storageContainer ? <StatusBadge value={specimen.storageContainer.temperatureZone} /> : '-'}</Descriptions.Item></Descriptions></section>
      <section className="detail-section"><Typography.Title level={4}>分装台账</Typography.Title><EntityTable<SpecimenAliquot> pagination={false} dataSource={specimen.aliquots || []} emptyTitle="暂无分装台账" columns={[{ title: '冻存管编号', dataIndex: 'tubeCode' }, { title: '体积', dataIndex: 'volumeMl', render: (value) => `${value} mL` }, { title: '批次', dataIndex: 'batch' }, { title: '登记人', dataIndex: 'registeredByName' }, { title: '登记时间', dataIndex: 'registeredAt', render: formatDateTime }, { title: '备注', dataIndex: 'notes', render: (value) => value || '-' }]} /></section>
