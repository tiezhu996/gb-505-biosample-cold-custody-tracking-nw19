...
function VolumeSummary({ specimen }: { specimen: Specimen }) {
  return (
    <Typography.Text>
      剩余 <Typography.Text strong>{specimen.volumeMl} mL</Typography.Text>
      {' '}/ 共 {specimen.aliquotCount} 管 / 接收 {specimen.initialVolumeMl || specimen.volumeMl} mL
    </Typography.Text>
  )
}
...
export function SampleDrawer({ specimen, open, onClose }: { specimen: Specimen | null; open: boolean; onClose: () => void }) {
  return (
    <Drawer width={720} title={specimen ? `样本 ${specimen.accessionNo}` : '样本详情'} open={open} onClose={onClose}>
      {specimen && <Tabs items={[
        { key: 'profile', label: '基本信息', children: <Descriptions bordered size="small" column={2}><Descriptions.Item label="保管状态"><CustodyBadge state={specimen.state} /></Descriptions.Item><Descriptions.Item label="样本类型">{specimen.sampleType}</Descriptions.Item><Descriptions.Item label="受试者编码">{specimen.subjectCode}</Descriptions.Item><Descriptions.Item label="研究协议">{specimen.protocolCode}</Descriptions.Item><Descriptions.Item label="当前保管人">{specimen.currentCustodian}</Descriptions.Item><Descriptions.Item label="接收时间">{formatDateTime(specimen.receivedAt)}</Descriptions.Item><Descriptions.Item label="体积/冻存管"><VolumeSummary specimen={specimen} /></Descriptions.Item><Descriptions.Item label="有效期">{formatDateTime(specimen.expiresAt)}</Descriptions.Item><Descriptions.Item label="冻存容器">{specimen.storageContainer ? `${specimen.storageContainer.code} · ${specimen.storageContainer.name}` : '待分配'}</Descriptions.Item><Descriptions.Item label="位置">{specimen.position || '-'}</Descriptions.Item><Descriptions.Item label="温区">{specimen.storageContainer ? <StatusBadge value={specimen.storageContainer.temperatureZone} /> : '-'}</Descriptions.Item><Descriptions.Item label="备注" span={2}>{specimen.notes || '-'}</Descriptions.Item></Descriptions> },
        {
          key: 'tubes', label: `冻存管台账 (${specimen.aliquotTubes?.length || 0})`,
          children: specimen.aliquotTubes?.length
            ? <div className="tube-ledger"><div className="tube-ledger-head"><span>管编号</span><span>体积 (mL)</span><span>登记人</span><span>登记时间</span></div>{specimen.aliquotTubes.map((tube) => <div className="tube-ledger-row" key={tube.id}><span>{tube.tubeCode}</span><span>{tube.volumeMl}</span><span>{tube.registeredByName}</span><span>{formatDateTime(tube.createdAt)}</span></div>)}</div>
            : <Typography.Text type="secondary">尚未登记分装冻存管</Typography.Text>,
        },
